...
<tbody class="xans-element- xans-board xans-board-notice-1002 xans-board-notice xans-board-1002 center">
	<c:if test="${nowPage==1 }">
		...
		<tr style="background-color: #FAFAFA; color: #555555;" class="xans-record-">
		
		<td></td>
		<td align=center>
		<strong>
		<a href="javascript:read('${dto.id }')" style="color: #555555;">${dto.id }</a>
		<span class="txtEm"></span></strong>
		</td> 
		<td>${dto.name }</td>
		<td class="">
		<span class="txtNum">
	<c:choose>
		...
		<td align=center>
		<strong>
		<a href="javascript:read('${dto.id }')" style="color: #555555;">
		${dto.id }
		</a>
...
<option value="name"<c:if test="${col == 'name' }">selected</c:if>>
NAME
</option>

<option value="email"<c:if test="${col == 'email' }">selected</c:if>>
EMAIL
</option>
